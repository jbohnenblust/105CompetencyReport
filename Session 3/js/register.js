...
let scooby=new Pet(1,"Scooby",50,"Male","Dane","Full Service","Shaggy","555-555-5555","dog");
let snoopy=new Pet(2,"Snoopy",50,"Male","Beagle","Shower","Charlie Brown","666-666-6666","dog");
let scrappy=new Pet(3,"Scrappy",50,"Male","Dane","Nails Cut","Shaggy","555-555-5555","dog");
let speedy=new Pet(4,"Speedy",50,"Male","Mixed","Shower","Bunny","777-777-7777","dog");
...
function register(){
    ...
    let thePet=new Pet(salon.nextId,inputName,inputAge,inputGender,inputBreed,inputService,inputOwner,inputPhone,inputType);
    salon.nextId += 1;
...
function deletePet(id){
    ...
    // 1 remove object from the array (salon.pets)

    for(let i=0; i<salon.pets.length; i++){
        let pet = salon.pets[i];
        if(pet.id == id){
            salon.pets.splice(i,1); //remove element at index [i], no native delete/remove function
        }
    }

    // 2 display existing pets
    displayPets();
}
